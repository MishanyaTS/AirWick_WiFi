...
function send_request(submit,server,state){
 ...
 ajax.get(server,{},function(responses) {
  ...
  // load('next');
  if (JSON.parse(responses).should_refresh) {
	  setContent('edit');
  }
 },true);
}
...
function get_time_manual (ip){
var m = new Date(Date.now());
var time_string = m.getTime()
/*
var date_string = m.getUTCFullYear() + "-" +

    ("0" + (m.getMonth()+1)).slice(-2) + "-" +

    ("0" + m.getDate()).slice(-2) + "Time" +
 ...
 val('times1', date_string);
 */
 send_request(ip,'/get_time?get_time='+ Math.round(time_string/1000));
}   
